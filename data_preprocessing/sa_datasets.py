...
def preprocess_crlm(args):
    # Load data
    data_filename = args['input_dir'] + 'crlm/crlm.csv'
    print(data_filename)
    df = pd.read_csv(data_filename, sep=',' )
    print(df)
    print(df.__len__())
    ...
    # Extract info
    df.loc[:, 'age'] = df.loc[:, 'age'].astype(int)
    df.loc[:, 'body_mass_index'] = df.loc[:, 'body_mass_index'].astype(int)
    df.loc[:, 'carcinoembryonic_antigen'] = df.loc[:, 'carcinoembryonic_antigen'].astype(int)
    df.loc[:, 'max_tumor_size'] = df.loc[:, 'max_tumor_size'].astype(int)
    df.loc[:, 'NASH_score'] = df.loc[:, 'NASH_score'].astype(int)
    df.loc[:, 'total_response_percent'] = df.loc[:, 'total_response_percent'].astype(int)
    df.loc[:, 'necrosis_percent'] = df.loc[:, 'necrosis_percent'].astype(int)
    df.loc[:, 'fibrosis_percent'] = df.loc[:, 'fibrosis_percent'].astype(int)
    df.loc[:, 'mucin_percent'] = df.loc[:, 'mucin_percent'].astype(int)
    df.loc[:, 'months_to_DFS_progression'] = df.loc[:, 'months_to_DFS_progression'].astype(int)
    df.loc[:, 'months_to_liver_DFS_progression'] = df.loc[:, 'months_to_liver_DFS_progression'].astype(int)

    
    df = df.drop(labels=["clinrisk_score"], axis=1)

    label = df[['vital_status']].astype(int)
    time = df[['overall_survival_months']].astype(int)
    df = df.drop(labels=["vital_status", "overall_survival_months"], axis=1)
